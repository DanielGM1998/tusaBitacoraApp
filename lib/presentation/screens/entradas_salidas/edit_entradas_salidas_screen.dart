...
class EditEntradasSalidasScreen extends StatefulWidget {
  static const String routeName = 'editEntradasSalidas';

  final String idapp;
  final String idRegistro;

  const EditEntradasSalidasScreen({
    Key? key,
    required this.idapp, required this.idRegistro,
  }) : super(key: key);

  @override
  State<EditEntradasSalidasScreen> createState() => _EditEntradasSalidasScreenState();
}

class _EditEntradasSalidasScreenState extends State<EditEntradasSalidasScreen>
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(height: size.width * 0.02),
                  // Firma
                  Text("Agregar firma: ", style: TextStyle(color: myColor, fontSize: isLandscape
                    ? size.width * 0.02
                    : size.width * 0.03)),
                  Signature(
  ...
  Future <bool> save(context, id_registro, fk_matricula, nombre, empresa_externo, evidencia_externo, List<String>? evidencia_url, List<String>? evidencias_url, List<String>? firma_url, fecha_entrada, fecha_salida, fecha, guardia, fila, vin, modelo, llave, distribuidor, ejes, origen, modeloTanqueIzq, cmTanqueIzq, modeloTanqueDer, cmTanqueDer, tipoTanque, nivel, equipamiento) async{
    ...
    bool carga = await uploadEvidencia(imagenes: todasLasRutas, vin: vin ?? "");
    ...
    if (response.statusCode == 200) {
      ...
      EasyLoading.dismiss();
      success(context, "Registro editado exitosamente");
      ...
      final http.Response response;
        ...
        response = await http.get(
          Uri(
            scheme: https,
            host: host,
            path: '/app/bitacora/getRegistroByID.php',
            queryParameters: {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          ...
          if (data != null && data is List && data.isNotEmpty) {
            ...
            if(registro['fecha_entrada'] != null){
              tipoRegistro=0;
              dateTime = DateTime.parse(registro['fecha_entrada']);
            }else if(registro['fecha_salida'] != null){
              tipoRegistro=2;
              dateTime = DateTime.parse(registro['fecha_salida']);
            }else{
              tipoRegistro=1;
              dateTime = DateTime.parse(registro['fecha']);
            }
            // vin
            if(registro['VIN'] != null){
              _vinController.text = registro['VIN'];
              vin = registro['VIN'];
            }
            // matricula
            if(registro['fk_matricula'] != null){
              _matriculaController.text = registro['fk_matricula'];
              matricula = registro['fk_matricula'];
            }
            // nombre
            ...
            // empresa
            if(registro['empresa_externo'] != null){
              _esExterno = true;
              _empresaController.text = registro['empresa_externo'] ?? "";
              empresa = registro['empresa_externo'] ?? "";
            }
            // evidencia
            if(registro['evidencia_externo'] != null){
              List<String> lista = List<String>.from(jsonDecode(registro['evidencia_externo']));
              evidencia = lista;
            }
            // modelo
            if(registro['modelo_unidad'] != null){
              _modeloController.text = registro['modelo_unidad'];
              modelo = registro['modelo_unidad'];
            }
            // llave
            if(registro['llave'] != null){
              _llaveController.text = registro['llave'];
              llave = registro['llave'];
            }
            // distribuidor
            if(registro['distribuidor'] != null){
              _distribuidorController.text = registro['distribuidor'];
              distribuidor = registro['distribuidor'];
            }
            // ejes
            if(registro['num_ejes'] != null){
              _ejesController.text = registro['num_ejes'];
              ejes = registro['num_ejes'];
            }
            // origen
            if(registro['origen'] != null){
              _origenController.text = registro['origen'];
              origen = registro['origen'];
            }
            // Modelo Tanque izq
            modeloTanqueIzq = registro['modelo_izq'] ?? 'Seleccione un modelo';
            modeloTanqueIzqPrint = registro['modelo_izq'];
            // cm tanque izq
            if(registro['modelo_izq']!='Seleccione un modelo'){
              showCmTanqueIzq = true;
            }else{
              showCmTanqueIzq = false;
            }
            if(registro['cm_tanque_izq'] != null){
              CMTanqueIzq = registro['cm_tanque_izq'];
              CMTanqueIzqPrint = registro['cm_tanque_izq'];
            }
            // Modelo Tanque der
            modeloTanqueDer = registro['modelo_der'] ?? 'Seleccione un modelo';
            modeloTanqueDerPrint = registro['modelo_der'];
            // cm tanque der
            if(registro['modelo_der']!='Seleccione un modelo'){
              showCmTanqueDer = true;
            }else{
              showCmTanqueDer = false;
            }
            if(registro['cm_tanque_der'] != null){
              CMTanqueDer = registro['cm_tanque_der'];
              CMTanqueDerPrint = registro['cm_tanque_der'];
            }
            // Tipo tanque
            tipoTanque = registro['tipo_tanque'];
            // nivel
            nivel = registro['nivel_urea'];
            // equipamiento
            if(registro['equipamiento'] != null){
              List<String> seleccionados = registro['equipamiento'].split(',');
              checkboxes.updateAll((key, value) => seleccionados.contains(key));
            }
            // fila
            fila = registro['fila'];
          }
          ...
          return PopScope(
            ...
            child: Container(
              ...
              child: RouteAwareWidget(
                screenName: "editEntradasSalidas",
                child: Scaffold(
                  key: _scaffoldKey,
                  backgroundColor: Colors.white.withOpacity(1),
                  drawer: SideMenu(userapp: _userapp ?? "0", tipoapp: _tipoapp ?? "1", idapp: widget.idapp),
                  appBar: AppBar(
                    title: Text(nameEditEntradasSalidas, style: TextStyle(color: Colors.white, fontSize: size.width * 0.04)),
                    elevation: 1,
                    toolbarHeight: 100,
                    centerTitle: true,
                                    ...
                                    onPressed: () async{
                                      if (_currentPage == _pages(context, size, isLandscape).length - 1) {
                                        ...
                                        if(tipoRegistro == 0){
                                          // entrada
                                          // ignore: use_build_context_synchronously
                                          await save(context, widget.idRegistro, matricula, nombre, empresa, jsonEncode(evidenciaNombres), imagePaths, imagePathsEvidencias, imagePathsFirma, dateTime.toString(), "", "", "${_userapp!} ${_apellidos!}", fila, vin, modelo, llave, distribuidor, ejes, origen, modeloTanqueIzqPrint, CMTanqueIzqPrint, modeloTanqueDerPrint, CMTanqueDerPrint, tipoTanquePrint, nivelPrint, seleccionadosCSV);                                          
